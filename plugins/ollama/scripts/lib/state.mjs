...
// ── Global config (OLLAMA is machine-scoped, not workspace-scoped) ──────────

const GLOBAL_CONFIG_DIR = path.join(os.homedir(), ".ollama-plugin-cc");
const GLOBAL_CONFIG_FILE = path.join(GLOBAL_CONFIG_DIR, "config.json");

export function loadGlobalConfig() {
  try {
    if (!fs.existsSync(GLOBAL_CONFIG_FILE)) {
      return { activeModel: null, ollamaEndpoint: "http://localhost:11434" };
    }
...
export function updateGlobalConfig(patch) {
  saveGlobalConfig({ ...loadGlobalConfig(), ...patch });
}

// ── Workspace-scoped job state (adapted from qwen-plugin-cc) ─────────────────
...
function pruneJobs(jobs) {
  return [...jobs]
    .sort((l, r) =>
      String(r.updatedAt ?? "").localeCompare(String(l.updatedAt ?? "")),
    )
    .slice(0, MAX_JOBS);
}
